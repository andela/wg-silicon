...
def add(request, pk):
    ...
    WorkoutLogFormSet = modelformset_factory(
        WorkoutLog,
        form=WorkoutLogForm,
        exclude=('date', 'workout', 'workoutsession_id'),
        extra=total_sets)
    # Process the request
    if request.method == 'POST':
        ...
        if dateform.is_valid() and session_form.is_valid(
            ...
            for instance in instances:
                ...
                instance.workout = day.training
                instance.date = log_date
                instance.workoutsession_id = workoutsession_id
                instance.save()
        ...
        for day_list in self.object.canonical_representation['day_list']:
            ...
            for set_list in day_list['set_list']:
                ...
                for exercise_list in set_list['exercise_list']:
                    ...
                    # reason (bug in django? DB problems?) when adding the
                    # filter there, the execution time explodes. The weight
                    # unit filter worksas expected. Also, adding the unit
                    # IDs to the exclude list also has the disadvantage
                    # that if new ones are added in a local instance, they
                    # could "slip" through.
