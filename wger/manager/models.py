    ...
    def get_current_workout(self, user):
        ...
        try:
            ...
            if schedule.schedulestep_set.count():
                ...
                # this is kind of wrong, but lets us continue to the
                # correct place
...
class Day(models.Model):
    ...
    def get_canonical_representation(self):
        ...
        for set_obj in self.set_set.select_related():
            ...
            # repetitions. If not, just take the smallest number and
            # drop the rest, becauseotherwise it doesn't make sense
        ...
        return {
            ...
            'days_of_week': {
                'text':
                    u', '.join([
                        six.text_type(
                            _(i.day_of_week)) for i in tmp_days_of_week
                    ]),
                'day_list':
                    tmp_days_of_week
            },
    ...
    def clean(self):
        ...
        if (not self.time_end
            ...
            raise ValidationError(
                _("If you enter a time, you must enter both start and"
                  " end time."
                  ))

        if self.time_end and self.time_start and self.time_start > self.time_end:  # noqa
            raise ValidationError(
...
class WorkoutLog(models.Model):
    ...
    workoutsession_id = models.ForeignKey(WorkoutSession,
                                          verbose_name=_('workoutsession_id'),
                                          null=True)
    ...
    def get_workout_session(self, date=None):
        ...
        try:
            if not self.workoutsession_id:
                return WorkoutSession.objects.filter(
                    user=self.user).get(date=date)

            workoutsession_id = self.workoutsession_id
            return WorkoutSession.objects.filter(user=self.user).get(
                date=date, id=workoutsession_id.id)
