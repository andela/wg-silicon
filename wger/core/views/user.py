    ...
    if user_pk:
        ...
        if not request.user.has_perm('gym.manage_gyms') \
                and (not request.user.has_perm('gym.manage_gym')
                     or request.user.userprofile.gym_id != user.userprofile.gym_id  # noqa
...
def trainer_login(request, user_pk):
    ...
    # - https://docs.djangoproject.com/en/1.6/topics/auth/default/#auth-web-requests  # noqa
    # - http://stackoverflow.com/questions/3807777/django-login-without-authenticating  # noqa
    if own:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (request.user.has_perm('gym.manage_gym')
            or request.user.has_perm('gym.gym_trainer')) \
                and edit_user.userprofile.gym_id != request.user.userprofile.gym_id:  # noqa
            return HttpResponseForbidden()
